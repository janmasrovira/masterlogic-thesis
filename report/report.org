...
Extensionality a b = {A : Set a} {B : Set b} {f g : A → B} →
  ((x : A) → f x ≡ g x) → f ≡ g
    ...
    $\sf{ZFC}$, however, in Agda the property of extensionality is not an axiom
...
*** Implicit arguments and inference
    ...
    arguments ={A} → B=. In the following =example=, the hidden argument to =scons= of
    type =zero ≤ zero= can be solved by \text{$η$-expansion}, since this type reduces to =⊤=.
