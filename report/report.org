...
* Generic generalized frame condition
  In this section we present a method that given a formula $A$, builds a (first
  order??) formula that is a generalized frame condition for $A$.

  {{{begindef}}} Given a generalized frame $F=⟨W,R,S⟩$ and a formula $A$, let
  $ℱ:(Var(A) → 𝒫(W))×Fm→𝒫(W)$ be defined by
\begin{flalign*}
  ℱ(G,x) &≔ G (x) \\
  ℱ(G,⊥) &≔ ∅ \\
  ℱ(G,A→B) &≔ \{w:w ∈ ℱ(G,A) ⇒ w ∈ ℱ(G,B)\} \\
  ℱ(G,A▷B) &≔ \{w:∀ u.(wRu,u∈ℱ(A))⇒∃Y.uS_wY⊆ℱ(B))\} \\
\end{flalign*}

  Then define
  \[(A)^*_{gen}≔∀(G:Var(A) → 𝒫(W))∀w∈W.w∈ℱ(G,A) \]

  Since $Var(A)=x₁,…,xₙ$ is finite, we can have instead
  \[(A)^*_{gen}≔∀𝕏₁…𝕏ₙ∀w∈W.w∈ℱ(𝕏₁,…,𝕏ₙ,A) \]
  where $G(xᵢ)$ is replaced by $𝕏ᵢ$ for $i∈\{1…n\}$.
