...
** Overview of interpretability logics
   ...
   recursively enumerable and are sequential (?). Moreover, we are only
   concerned with the provable, within $T$, interpretability properties. In
   other words, we study the behaviour of the binary interpretability predicate
   $\sf{Int}_T$. Naturally, the predicate $\sf{Int}_T(A,B)$ must hold when we
   can formalize in $T$ that $T+A$ interprets $T+B$. For a theory $T$, we denote
   the logic that describes the behaviour of the interpretability predicate of
   $T$ as $\sf{IL}(T)$. The language of $\sf{IL}(T)$ extends the language of
   \sf{GL} with the binary modal operator $▷$, thus the language becomes
   $⟨⊥,→,□,▷⟩$. In order to give a more detailed definition of $\sf{IL}(T)$ we
   first define what is an /arithmetical realization/. We say that a map $*$
   from formulas in $\sf{IL}(T)$ to sentences in the language of $T$. Moreover,
   it must satisfy the following constraints:
   \begin{flalign*}
   ⊥^* \text{ is mapped to a contradiction;} \\
   (A→B)^*= A^*→B^* ; \\
   (A▷B)^*= $\sf{Int}_T(⌜A⌝,⌜B⌝)$ ; \\
   \end{flalign*}


   mention \sf{M}, $P$ and $R$.
** Original contributions
   ...
   7. we give details for an embedding of propositional intuitionistic logic
      ...
      were unable to find somewhere else. [[sec:def-new-types][Link]];
   8. during the development of this project we have coauthored two publications:
   ...
   9. we have implemented an Agda library for interpretability logics which
   ...
   10. we have reimplemented a small portion of the Agda library in the Coq proof
...
# SHORTLY WE WILL PUT IT ON THE ARXIV SO THAT YOU CAN INCLUDE A REFERENCE
 ...
 {{{jan(rename M to avoid clash with montaga's principle)}}}
