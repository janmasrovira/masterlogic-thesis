...
** The principle \prin{P₀}
   ...
   during the search the modal completeness proof of \prin{ILM_0}. In an attempt
   ...
   $\prin{ILP_0}⊬\prin{ILM_0}$ and thus we can conclude that $\prin{ILP_0}$ is
   modally incomplete.

   TODO: mention that ILP0 is in IL(ALL).
