...
** Overview of interpretability logics
   ...
   Veltman) and /generalized Veltman semantics/ (introduced by Rineke Verbrugge
   in cite:Verbrugge). Ordinary Veltman semantics are defined in much the same
   spirit as Kripke semantics for \sf{GL}. However, ordinary Veltman frames have
   an additional family of relations indexed by each of the
