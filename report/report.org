...
data _,_⊮_ M w where
  ...
  bot : M , w ⊮ ⊥'
   ...
   1. =∀ {M w A} → M , w ⊩ A ⇔ ¬ (M , w ⊮ A)=.
   1. =∀ {M w A} → ¬ (M , w ⊩ A) ⇔ M , w ⊮ A=.
   For lemma 1 we can prove $⇒$ and for lemma 2 we can prove $⇐$. However, it is
   not possible to prove the remaining directions. In general terms, this is due
   to the fact that in Agda (and in intuitionistic logic in general) we can
   prove that =(¬ A ⊎ B) → A → B= but we cannot prove =A → B → (¬ A ⊎ B)= since
   we lack the non-constructive law of excluded middle.
