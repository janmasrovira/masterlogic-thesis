...
*** Generalized Veltman semantics
    ...
    quasi-transitivity property given in Definition [[def:gen-frame]]. However, we
    ...
    follows that there exists $V$ such that $uS_wV⊩B$. By Condition 3 we have
    that $∀v∈V.wRv⊩B$. Then for every $v∈V$ we have that there exists $Z_v$ such
    that $vS_wZ_v⊩C$. It follows from the quasi-transitivity condition that
    $uS_w(⋃_{v∈V}Z_v)$ and clearly $⋃_{v∈V}Z_v⊩C$. {{{endproof}}}
