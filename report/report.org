...
** Overview of interpretability logics
   ...
   of view which birthed the axiomatization of the propositional provability
   ...
   a /frame condition/ for a principle \sf{X} is a first (or higher) order
   formula such that for every Veltman frame $F$ we have \[F⊨𝒞 ⇔ ⟨F,V⟩⊩\sf{X}
   \text{ for any valuation $V$ and instance of \sf{X}.} \] To give an example,
   ...
   The principle \prin{M} is defined as follows: \[\sf{M}≔(A ▷ B) → ((A ∧ □ C) ▷
   (B ∧ □ C)).\] Then, consider the following condition:
   #+begin_center
...
| (8) | $uS_xY ⇒ ∀\, y∈Y\, ∀ Y'(yS_xY'\wedge y∉Y' ⇒ uS_xY')$                                   | Goris, Joosten '09 \cite{a-new-principle}     |
 ...
 easy to observe that $M'$ satisfies Conditions 1--5 from cref:def:gen-frame. It
 is also easy to see that it satisfies quasi-transitivity (2). We show that they
 force the same formulas by induction on the complexity of the formula. The only
