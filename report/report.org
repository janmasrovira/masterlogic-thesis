...
** Overview of interpretability logics
   ...
   completeness ({{{jan(REF needed)}}}) proofs. As we have already mentioned,
   \il{} is just the logic that we use as a base in interpretability logics,
   thus, each time we extend it with an axiom scheme (or principle, as we call
   them) we will need to find new semantics for it. Given a collection of
   principles, we get the semantics for the extension of \il{} with these
   principles by finding their frame condition. More precisely, a /frame
   condition/ for a principle \sf{P} is a first (or higher) order formula such
   that for every Veltman frame $F$ we have \[F⊨𝒞 ⇔ ⟨F,V⟩⊩\sf{P} \text{ for any
   valuation $V$ and instance of \sf{P}.} \] To give an example, let us put our
   focus on the interpretability logic of \sf{PA}. It happens that the
...
** Original contributions
   ...
   10. we have implemented an Agda library for interpretability logics which
       ...
       \end{center}
       We also provide the code in the annex of this thesis. [TODO: ADD LINK]
...
** Logic \il
   ...
   language (given by L and K) plus some new axiom schemas:
   - C1: $A → (B → A)$;
   - C2: $(A → (B → C)) → ((A → B) → (A → C))$;
   - C3: $(¬ A → ¬ B) → (B → A)$;
   - K: $□ (A → B) → □ A → □ B$;
   ...
   We proceed by showing a number of theorems related to the logic \il{}. All of
   these theorems have been verified and their Agda proof can be found in
   [[src:il.properties]].
...
*** Generalized Veltman semantics
    ...
    6. /quasi-transitivity/: if $uS_wY$ and $yS_wZ_y$ for all $y∈Y$, then
       ...
       quasi-transitivity, throughout this thesis we explore a total of eight
...
**** Identity set
     ...
     *Notation*. We will write $A\ ≡_S\ B$ instead of $≡\ S\ A\ B$.
     ...
     $\el{ℕ}↠\set$ as required by the type of $\sf{case}_ℕ$.
     ...
     that this can be done thanks to the \(\text{$α$-$=$}\) rule.] $((m:\el{ℕ})↠
