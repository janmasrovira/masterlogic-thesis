...
** Basic Agda
  ...
  #+begin_example
    data D (x₁ : P₁) … (xₖ : Pₖ) : (y₁ : Q₁) → … → (yₗ : Qₗ) → Set ℓ where
      ...
      cₙ : Tₙ
    #+end_example
  Note that it is possible to have $k=0$, $l=0$ and/or $n=0$.
...
*** Contexts, and typing rules
    ...
    {{{begindef}}} *Context*. A context is a (finite) set of pairs
    $⟨identifier:type⟩$. It is used to keep track of what identifiers are bound
    ...
    *Notation*. We use $Γ;t:A$ as short for $Γ∪\{⟨t:A⟩\}$.
    {{{enddef}}}

    {{{begindef}}} <<def:agda-well-typed>> *Well-typed term*. We say that a term
    $t$ is well-typed in context $Γ$ if $Γ⊢t:A$ for some type $A$. The rules for
    $⊢$ are presented below. We also define the relation $⊢_{Ρ}$, which is for
    typing patterns. For that purpose we need an auxiliary function definition,
    $τ$, which is defined afterwards.
   ...
   \inferrule[Pattern abstraction]{\text{Let } \overline{D}≔D\ x₁\ …\ xₙ\ t₁\ …\ tₙ \\\\
     ...
     Γ∪τ(Γ,\overline{D},p₁)⊢ s₁:B \\ … \\ Γ∪τ(Γ,\overline{D},pₙ)⊢ sₙ:B}
   ...
   \and
   \inferrule[Wildcard]{\ }{Γ⊢_{Ρ}\_ : A}
   \\
   \inferrule[Constructor]{c:(b₁:B₁)→…→(b:Bₙ)→D\ x₁\ …\ xₙ\ t₁\ …\ tₙ ∈Γ \\\\ Γ⊢_Ρp₁: B₁ \\ … \\ Γ⊢_Ρpₙ:Bₙ   }{Γ⊢_Ρc\ p₁\ …\ pₙ : D\ x₁\ …\ xₙ\ t₁\ …\ tₙ}
   \end{mathpar}
   \end{figure}

   Let $\overline{D}≔D\ x₁\ …\ xₙ\ t₁\ …\ tₙ$. We now define
   $τ(Γ,\overline{D},p)$, which is the of identifiers bound by pattern $p$ and
   their respective types. Assume that $Γ⊢_{Ρ}p:\overline{D}$. Finally, let $τ$
   be defined as follows:
  \begin{flalign*}
  &τ(Γ,\overline{D},x)≔ \{x:\overline{D}\} & \text{Identifier} \\
  &τ(Γ,\overline{D},\_)≔ ∅ & \text{Wildcard} \\
  &τ(Γ,\overline{D},c\ p₁\ …\ pₙ)≔ τ(Γ,p₁,B₁)∪…∪τ(Γ,pₙ,Bₙ) & \text{Constructor} \\
  &\hspace{3.5cm} \text{assuming } c:(b₁:B₁)→…→(b:Bₙ)→\overline{D}∈Γ
  \end{flalign*}


  Absurd pattern {{{jan(TODO)}}}.
...
*** Normalization
    ...
    Normalization is refers to the process of simplifying or evaluating a term
    via rewrite rules. The reader may be already acquainted with the
    \textsc{$β$-reduction} rule, which is present in untyped lambda calculus,
    the most basic form of lambda calculus. Here we present the mentioned rule
    among others. We use the notation $t↓t'$ to say that term $t$ normalizes to
    term $t'$. Normalization can happen in nested terms. For instance, if we
    have that $t↓t'$ then $λ x→ t↓λx→t'$. Likewise for the other terms.
   ...
   #+attr_latex: :float
    \begin{figure}[H]
    \begin{mathpar}
    \inferrule[$β$-reduction]{\ }{(λ x → t)\ a↓t[x↦a] } \and
    \inferrule[Transitivity]{a↓b \\ b↓c }{a↓c} \and
    \inferrule[Definition]{f=t∈Δ  }{Δ⊢f↓t}
    \\
    \inferrule[Match]{a↓b \\ ∃^{min}i.match(pᵢ,b)=σ  }{Δ⊢λ\ \{p₁→t₁;…;pₙ→tₙ\}\ a↓σ(tᵢ)}
    \end{mathpar}
    \end{figure}

    Here we define the partial function $match$, which takes a pattern and a
    term and returns (maybe) a substitution. We represent a substitution by a
    set of pairs of the form $⟨x↦t⟩$, which means ``replace identifier $x$ by
    term $t$''. Keep in mind that a pattern does not contain repeated
