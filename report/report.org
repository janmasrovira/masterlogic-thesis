...
*** Generalized semantics
   ...
   \[ ∀u,w,V(uS_wV⇒ ΣV'⊆V(uS_wV',∀v'∈V'∀z(v'Rz⇒uRz)))\]
   ...
   - \boxed{⇐} Let $a,b,c∈Var$ and assume $F⊩a ▷ b → (a ∧ □ c) ▷ (b ∧ □ c)$ and
     $uS_wV$. Assume for a contradiction that $(⋆)\ ∀V'⊆V(uSwV'⇒
     ∃v∈V'∃z(vRz,u\cancel{R}z))$. Consider a model satisfying the following
     \begin{flalign*}
     [a] &= \{u\} \\
     [b] &= V \\
     [c] & \{v:uRv\} \\
     \end{flalign*}
     We see that $w⊩a▷b$ since $a$ is only true in $u$ and we have $uS_wV⊩bS$.
     It follows that ${w⊩(a ∧ □ c)▷(b∧□c)}$. It is easy to see that $u⊩a∧□c$,
     hence there must exist $V'$ such that $uS_wV'⊩b∧□c$. Clearly $V'⊆V$, hence
     by $(⋆)$ it follows that there exists $v∈V'$ and $z$ such that $vRz$ and
     $u\cancel{R}z$. By the fact that $vRz$ it follows $z⊩c$ and by the fact
     that $u\cancel{R}z$ it follows $z⊮c$, which is an absurd.
   {{{endproof}}}
** $R$ principle
...
*** Ordinary semantics
    ...
    \[∀w,x,y,z,u,v,m(wRxRyS_wzRu⇒(yS_xu,((uS_xv,vRm)⇒uS_zm)))\]
    ...
    - \boxed{⇐} Let $a,b,c,d,e∈Var$ and assume $F⊩ a ▷ b → ((¬ (a ▷ ¬c) ∧(d▷♢e))
      ▷ (b ∧ □ c ∧ (d▷e)))$. Consider some worlds $w,x,y,z,u,v,m$ and assume
      $wRxRyS_wzRu$. Now consider a model based on $F$ that satisfies the
      following:
      \begin{flalign*}
      [a] &= \{y\} \\
      [b] &= \{z\} \\
...
*** Generalized semantics
    ...
    - \boxed{⇐} Assume for a contradiction that $F⊭(R₁)_{gen}$. It follows that
      ...
      \[(∀V'⊆V)(xS_wV', R[V']⊆K⇒ (∃v∈V')(∃c∈K)(vRcS_xR_x^{-1}[𝔼𝔼], (∀𝔼'⊆𝔼)
      c\cancel{S}_v 𝔼'))\]
