...
** Original contributions
   ...
   1. Generalized frame condition for $R₁$. [[theorem:R₁][Link]].
...
*** Generalized semantics
    ...
    - \boxed{⇒} Fix a model and assume that for some world $w$ we have $w⊩A▷B$.
      ...
      that there exist $y,z,x₀,…,x_{n-1}$ satisfying $1…6$. Then let $𝔸≔⟦A⟧$,
      $𝔹≔⟦B⟧$, $ℂ≔⟦C⟧$ and for $i<n$ let $𝔻ᵢ≔⟦Dᵢ⟧$. It is routine to check that
      the left part of the $(Rⁿ)_{gen}$ holds and thus we get that there exists
      some $V⊆𝔹$ such that $x_{n-1}S_wV$ and $R[V]⊆ℂ$. Since $V⊆𝔹$ we have that
      $x_{n-1}⊩B$ and since $R[V]⊆ℂ$ we have $x_{n-1}⊩□C$. Finally, since
      $x_{n-1}=x$ we conclude $x⊩B∧□C$.
    - \boxed{⇐}
    {{{endproof}}}
