...
** Monotonicity
  ...
  {{{begintheorem}}} <<theorem:mono-1>> Let $M=⟨W,R,S,V⟩$ be a generalized Veltman model with any
  of the quasi-transitivity conditions presented in section [[sec:trans]]. Let
  $M'=⟨W,R,S',V⟩$ where $S'$ is the monotonic closure of $S$. Note that $M'$ is
  not necessarily a Veltman model.
  ...
  {{{begintheorem}}} <<theorem:mono-1>> Let $F=⟨W,R,S⟩$ be a generalized Veltman frame
  with quasi-transitivity (2) (see [[sec:trans]]). Let $F'=⟨W,R,S'⟩$ where $S'$ is the
  monotonic closure of $S$.

  \[S'≔\{⟨w,x,Y'⟩ : ⟨w,x,Y⟩∈S, Y⊆Y'⊆\{u:wRu\}\}\]

  Then for any valuation $V$, let $M≔⟨F,V⟩$, $M'≔⟨F',V⟩$. For any world $w$ and
  formula $A$ we have \[M,w⊩A⇔M',w⊩A;\] additionally, $F'$ is a Veltman
  frame with quasi-transitivity condition (2). {{{endtheorem}}}


  {{{beginproof}}} The first part follows from theorem [[theorem:mono-1]]. For the
  ...
  - condition 3 follows from the fact that in the definition of $S'$ we require
    $Y'⊆\{u:wRu\}$;
  - for conditions 4 and 5 observe that $S⊆S'$. Since these condition hold for
  ...
  - for quasi-transitivity (2) assume that $uS'_xY'$ and that for every $y'∈Y'$
    ...
    $y'∈Y'$ we there exists some $Y_{y'}⊆Y'_{y'}$ such that $y'S_xY_{y'}$. By
    (2) for $F$ it follows that $uS_x⋃_{y'∈Y'}Y_{y'}$ and since $S⊆S'$ we get
    $uS'_x⋃_{y'∈Y'}Y_{y'}$.
  {{{endproof}}}
  ...
  As Luka pointed out during online correspondence, theorem [[theorem:mono-1]] does
  not hold in general for all quasi-transitivty conditions. For instance, here
...
** Transitivity
 ...
 (3) &$uS_xY → ∃\, y∈Y\, ∀ Y'(yS_xY' → ∃ \, Y''{⊆}Y' ∧ uS_xY'')$ & This paper \\
  \hline
 (4) &$uS_xY → ∃\, y∈Y\, ∀ Y'(yS_xY' → uS_xY')$ & \cite{joosten-master}  \\
 \hline
 ...
 \hline
 (8) & $uS_xY → ∀\, y∈Y\, ∀ Y'(yS_xY'\wedge y∉Y' → uS_xY')$ & \cite{a-new-principle}  \cite{interpretability-formalized}   \\
 \end{tabular}
 \end{center}
