...
** Original contributions
   ...
   4. every Agda proof.
   5. Embedding of intuitionistic logic into Martin Löf's logical framework.
      This result is expected. Our contribution has been to provide detailed
      definitions and proofs, which we were unable to find somewhere else.
...
**** Function set
     ...
     *Notation*: We will use
     ...
     \[\sf{case_Λ} : (A:\set)↠(B:\set)↠(\el{A↣B})↠ \el{A}↠\el{B} \]
     And the equality:
     \begin{mathpar}
     \inferrule[case$Λ$]{
...
**** Bottom set
     ...
     Let us now show that the principle $(⊥→p)^*$ know as /ex falso quodlibet/
...
**** Pairs set
     ...
     *Notation*. We will use infix notation for $×$, hence we will write $A\ × B$ instead
     ...
     Add the following equation to the $⟦.⟧$ map:
     \begin{flalign*}
     ⟦A∧B⟧≔⟦A⟧×⟦B⟧&&
     \end{flalign*}
     ...
     \end{mathpar}
     Lastly we show that $p→(q→(p∧q))^*$ is a theorem in LF.
     \begin{mathpar}
