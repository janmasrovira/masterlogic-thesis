...
** Basic Agda
   ...
   is an incomplete simplification and thus is not meant to be a
   precise reference for the real Agda language.
...
*** Contexts, and typing rules
  ...
  {{{begindef}}} <<def:agda-check-module>> *Type checking a module*. We
  understand by /type checking/ the process of checking that all terms in a
  module are well-typed and respect the typing annotations. Below we describe
  the process in detail.
    ...
    the rules. However, for simplicity we assume that the annotation is always
    required.
...
*** Normalization
    ...
    We use $a,b,c,t$ to denote terms and $f$ to denote an identifier.
