...
data _,_⊩_ (M : Model) (w : MW M) : Fm → Set where
  ...
  impl : {A B : Fm} → ((M , w ⊩ A) → (M , w ⊩ B)) → M , w ⊩ (A ↝ B)
   ...
   for /forcing/ (=_,_⊩_=) and /not forcing/ (=_,_⊮_=). The idea is the
   following; take the =impl= constructor as an example. Initially we had
   =impl : {A B : Fm} → ((M , w ⊩ A) → (M , w ⊩ B)) → M , w ⊩ (A ↝ B)=


   Hence we can define the following in
   Agda.
   #+begin_src text
