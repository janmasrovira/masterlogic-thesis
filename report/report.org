...
#+macro: agda @@latex:\includegraphics[height=\baselineskip]{img/agda}@@
...
** Ordinary Veltman semantics
   ...
   relation $S⊆W×W×W$. We write $wRu$ instead of $⟨w,u⟩∈R$ and
   $uS_wv$ instead of $⟨w,u,v⟩∈S$. The structure must satisfy the following
   conditions:
   ...
   $M,w⊩A$ instead of $⟨w,A⟩∈⊩_M$ or simply $w⊩A$ when the model is clear by
...
** Generalized Veltman semantics
   {{{begindef}}} An generalized Veltman frame $F=⟨W,R,S⟩$ is a structure
   constituted by a set of worlds $W$, a binary relation $R⊆W²$ and a ternary
   relation $S⊆W×W×(𝒫(W)∖\{∅\})$. We write $wRu$ instead of $⟨w,u⟩∈R$ and
   $uS_wY$ instead of $⟨w,u,Y⟩∈S$. The structure must satisfy the following
   conditions:
   ...
   5. if $xS_wY$ and $yS_wZ$ and $y∈Y∖Z$ then $xS_wZ$;
   6. if $wRu$ and $uRv$ then $uS_w\{v\}$;
   7. $S$ is monotone in the following sense: if $uS_wV⊆Z⊆\{u:wRu\}$ then
      $uS_wZ$.
   {{{enddef}}}

   {{{begindef}}} A generalized Veltman model $M=⟨F,V⟩$ is a structure
...
*** Generalized semantics
   ...
   \[ ∀w,x,V(xS_wV⇒ ΣV'⊆V(xS_wV',∀v'∈V'∀z(v'Rz⇒xRz)))\]
...
*** Ordinary semantics
    ...
    The $(R₁)_{ord}$ frame condition reads as follows (pull quantifiers out?):
...
*** Generalized semantics
    ...
    &wRx₀R…Rx_{n-1}RyRz, \\
    & (∀u.wRu,u∈𝔸⇒∃V.uS_wV⊆𝔹), \\
    & (∀u.x₀Ru∈𝔻_{n-1}⇒∃V.uS_{x₀}V⊆𝔸), \\
    & (∀i∈\{1…n-1\}∀u.xᵢRu∈𝔻_{n-i-1}⇒∃V.uS_{xᵢ}V⊆𝔻ₙ), \\
    & (∀V.zS_yV⇒V∩ℂ≠0),      \\
    & z∈𝔻₀ \\
    ⇒\ & ∃V⊆𝔹.x₀S_wV,R[V]⊆ℂ
    \end{flalign*}
    \newpage
*** Generalized semantics 2
    The $(Rⁿ)_{gen}$ condition reads as follows:
    ...
    &wRx_{n-1}R…Rx_0RyRz, \\
    & (∀u.wRu,u∈𝔸⇒∃V.uS_wV⊆𝔹), \\
    & (∀u.x_{n-1}Ru∈𝔻_{n-1}⇒∃V.uS_{x₀}V⊆𝔸), \\
    & (∀i∈\{1…n-1\}∀u.xᵢRu∈𝔻_{n-i-1}⇒∃V.uS_{xᵢ}V⊆𝔻ₙ), \\
    & (∀V.zS_yV⇒V∩ℂ≠0),      \\
    & z∈𝔻₀ \\
    ⇒\ & ∃V⊆𝔹.x₀S_wV,R[V]⊆ℂ
...
* Agda in the project
  Everything about agda in the project.
