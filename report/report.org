...
*** BHK interpretation of propositional logic
 ...
 data _⊎_ (A B : Set) : Set where
   ...
   wrong : (inj₁ a) = a
   #+end_src
   The answer is that Agda complains. As explained in cref:sec:agda-totality
   Agda requires all of the definitions to be total, and thus when we split an
   argument into cases using pattern matching, it checks that the cases are
   exhaustive.
