...
*** A guided Agda proof
    ...
    proof. This is intended to give the reader a feel of how an Agda proof
    dealing with semantics. We prove that for any generalized Veltman model $M$ and world
    $w$ we have that $w$ forces Löb's axiom. In symbols: \[M , w ⊩ □ (□ A → A) →
    □ A.\] Note that this is part of the soundness proof for IL.
    ...
    copy their type here and we omit their proof in order to save space. Also,
  ...
  InfiniteChain.a<b (L-chain {w} {u} Rwu uA uF) = Rwu
    ...
    the =width= construct[fn::The =width= construct allows us to pattern match
    ...
    the =⊩→¬⊮= lemma. Then we can use the principle of explosion to return
    anything.
    #+begin_src
  InfiniteChain.tail (L-chain Rwu u⊮A w⊩□⟨□A↝A⟩) with (⊩□ ⇒ w⊩□⟨□A↝A⟩) Rwu
