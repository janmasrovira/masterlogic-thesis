...
* Verbrugge
  ...
  \begin{flalign*}
  (C₀)&≔∀wxV.xS_wV⇒∃y∈V.∀bV'.SbyV'⇒∃v∈V'. (b=w ⇒ xS_b\{v\}), (bRw ⇒ wS_b\{v\}); \\
  (C₁)&≔∀wbxV.wRx⇒xS_bV⇒∃v∈V.(b=w ⇒ xS_b\{v\}),(bRw⇒wS_b\{v\}).
  \end{flalign*}
  {{{begintheorem}}}
  ...
  - \boxed{⇒} Assume $w⊩D▷E$ and let $C$ be such that $⟨w,C⟩∈W'$. We
    ...
    $(W2)$ for $A$ it follows $⟨w,v⟩∈A$. We proceed likewise and use
  ...
  # OLD
    # By quasi-transitivity Condition (4) on $xS_wV$ we have
    # that there exists $y∈V$ such that \[ (E1)\ \ ∀Z(yS_wZ⇒xS_wZ). \] Then let
    # \begin{flalign*}
    # B&≔\{⟨u,v⟩:∃Y(yS_uY,v∈Y), u=w⇒⟨w,v⟩∈A\}
    # \end{flalign*}
    # We show that $⟨y,B⟩∈W'$.
    # such that ⟨y,B⟩∈W' and ⟨w,v⟩∈B⇒⟨w,v⟩∈A
    # 1. (W1) By definition of $B$.
    # 2. (W2) Assume that for some $u,V$ we have $yS_uV$. Then clearly
    #    $∃Y(yS_uY,v∈Y)$. Now we show $⟨w,v⟩∈A$. Since we have that $yS_wV$ by
    #    $(E1)$ it follows that $xS_wV$ and by $(W2)$ there is some $v'$ such that
    #    $⟨w,v'⟩∈A$.
    # Then by IH we have $⟨y,B⟩⊩E$ so
    ...
    1. $∀v(⟨w,v⟩∈B⇒⟨w,v⟩∈A)$. By definition of $B$.
  - \boxed{⇐} We proceed by contraposition. Assume $w⊮D▷E$, then there exists
    $x$ such that $wRx$ and $∀Y(vS_wY⇒∃y∈Y(y⊮E))$.
  {{{endproof}}}
* other
