...
data Σ {ℓ ℓ' : Level} (A : Set ℓ) (B : A → Set ℓ') : Set (ℓ ⊔ ℓ') where
   ...
   *A note on syntax*. The reader may find the $Σ$ syntax a bit too different
   from what we use in paper. We can fix that thanks to Agda's syntax
   versatility. Agda provides a tool to define custom syntax. Below we show how
   we can use that tool to improve the syntax of $Σ$. We will not go into more
   detail since this is feature is of shallow mathematical interest.
   #+begin_src text
Σ-syntax : {ℓ ℓ' : Level} → (A : Set ℓ) → (A → Set ℓ') → Set (ℓ ⊔ ℓ')
Σ-syntax = Σ
syntax Σ-syntax A (λ x → B) = Σ[ x ∈ A ] B
   #+end_src
   With this syntax enhancement we can replace =Σ A P= by =Σ[ c ∈ A ] (P c)=.
...
** Generalized semantics
   ...
   defining a datatype without a quasi-transitivity condition. See that we
   define the type =𝕎 ≔ Pred W ℓW=, which means that a term of type =𝕎= is a
   subset of =W= (see section [[sec:sets]] for details on how to represent
   mathematical sets in Agda).
   #+begin_src text
