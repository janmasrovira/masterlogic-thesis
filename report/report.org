...
*** Normalization
    ...
    simplifying or evaluating a term via rewrite rules. Normalization is done
...
**** Pattern matching on indexed datatypes
     ...
     =b=. Then filling the case where =b= is equal to =true= is straightforward
     because then we must provide a term of type =IsFalse (not true)=.
     #+begin_example
     property : (b : Bool) → (p : IsTrue b) → IsFalse (not b)
     property = λ {true → λ p → is-false; false → λ p → ? }
     #+end_example
