...
** Overview of interpretability logics
   ...
   significance of the new axiom schemas in Section [[sec:il]].
...
** Original contributions
   ...
   4. we analyzed how a monotonicity condition for generalized semantics that is
      ...
      literature. Furthermore, we justify why assuming such condition if it is
      not required by definition is harmless. [[sec:mono][Link]];
   5. we discovered a proof in a published article which needs to be repaired. [[sec:flawed-proof][Link]];
   6. we compare the expressiveness of ordinary and generalized Veltman models.
      ...
      manuscript in Section [[pdf:Verbrugge]]. [[sec:verbrugge][Link]];
   ...
   9. during the development of this thesis we have coauthored two publications:
      ...
      - /Generalised Veltman Semantics in Agda/ (cite:masrovira2020generalised).
        In this publication we focus on the frame conditions for
        generalized Semantics for the principle \prin{R_1} and the series
        \prin{R^n}. We also comment on the Agda formalization.
...
** Quasi-transitivity
   ...
   that in definition [[def:gen-frame]] we used Condition (2). Theorem [[theorem:trans]]
   presents some direct implications between them. Theorems [[theorem:il-sound]] and
...
* Frame conditions
  ...
  factors: On the one hand we have principles such as \prin{M} that give rise to
  interpretability logics for certain theories, for instance, \prin{ILM} is the
  axiomatization of the interpretability logic for \pa{}: \prin{IL(PA)}. On the
  ...
  \prin{R_n}, which we will present in this part, appear in the best known lower
  bound for \ilall. Regardless of the area of interest finding the frame
  condition for a principle is always the first step towards studying the nature
  of the principle using either Veltman semantics. Once we have established a
  ...
  utility grows when applying it to prove independence results between
  ...
  more precise, assume that we have a principle \prin{X} which does not
  necessarily hold in an arbitrary ordinary Veltman frame. A frame condition is
  ...
  write \kord{X} and \kgen{X} to denote the frame conditions ordinary and
  generalized semantics respectively.

  In Section [[sec:generic-cond]] we will verify in Agda that there is a mechanical
  ...
  formulation as the /original/): \[∀x.∀y.∀z.xRyRz⇒xRz\ ;\] \[∀𝕊.𝕊≠∅⇒∃s∈S∀s'
  ...
  is presented in detail in Section [[sec:generic-cond]].](we will refer to this
  formulation as the /generic/):
  \[∀𝔸.∀u(wRu⇒(∀u'(wRu'⇒u'∈𝔸))⇒u∈𝔸)⇒∀v(wRu⇒u∈𝔸).\] If we compare the original
  ...
  we find it impossible to objectively justify why the original formulation is
  preferred in the literature over the generic formulation. However, the case is
  that we /do/ have the concepts of transitivity and Noetherian relations in our
  commonly used metalanguage and moreover we are used to operating with them.
  Furthermore, it is easy for us to imagine a transitive relation that has no
  infinite ascending chains. On the contrary the picture that is described by
  the generic condition is rather blurry for us. All in all, we see that we can
  precisely define what is the minimal that a frame condition should be, but the
  quantification of how good a frame condition is remains open to
  interpretation. However, we will lean towards conditions which are easy to
  visualize.
