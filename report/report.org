...
* Frame conditions
  ...
  commonly used metalanguage and we are used to operating with them. Moreover,
  it is easy for us to imagine a transitive relation that has no infinite
  ascending chains. On the contrary the picture that is described by the generic
  condition is rather blurry for us. All in all, we see that we can precisely
  define what is the minimal that a frame condition should be, but the
