...
** Overview of interpretability logics
   ...
   to model the behaviour of the $▷$ binary modal operator. In generalized
   ...
   such that for every Veltman frame $F$ we have \[F⊨𝒞 ⇔ ⟨F,V⟩⊩P \text{ for any
   ...
   $\sf{ILM}$. The \prin{M} principle is defined as follows: \[\sf{M}≔A ▷ B → (A
   ∧ □ C) ▷ (B ∧ □ C).\] We give the definition for illustrative purposes, but
   ...
   \prin{R_n}, which appear in the current best known lower bound for \ilall.
