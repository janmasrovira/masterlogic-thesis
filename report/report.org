...
Sufficiently strong arithmetical theories such as \pa{} can formalize the
...
Interpretability logics extend provability logic \gl{} and thus inherit a number
of similarities from such logic, for instance, the possiblity to give relational semantics.
...
In this thesis we compare the expressivness of ordinary and generalized semantics
and give procedures that under some assumptions transform, while preserving theoremhood,
ordinary models to generalized models and vice versa.

We study a eight different versions of \textit{generalized Veltman semantics}
...
** Logic \il
   ...
   necessitation, C1 and MP we can prove $$. If $B$ follows from an
...
⊢A▷A {A} =
  begin[ 0 ] A ↝ A                        By ⊢A↝A
   ...
   transform proofs in the nice syntax to proofs in the original format. We will
