...
data Σ (A : Set) (B : A → Set) : Set where
  _,_ : (a : A) → (b : B a) → Σ A (B a)
    ...
    For instance:
    #+begin_src text
    show meaningful example of dependent pair
    #+end_src
...
data _,_⊮_ M w where
  ...
  bot : M , w ⊮ ⊥'
   ...
   However, it is not possible to prove the remaining directions. In general
   terms, this is due to the fact that in Agda (and in intuitionistic logic in
   general) we can prove that =(¬ A ⊎ B) → A → B= but we cannot prove =A → B →
   (¬ A ⊎ B)=. The reason being that we lack the law of excluded middle, as it
   is a non-constructive axiom. In order to prove the remaining directions we
   need to assume that the forcing relation is decidable.

   {{{begindef}}} We say that a model =M= is decidable if for any world =w= and
...
DecidableModel M = ∀ w A → M , w ⊩ A ⊎ M , w ⊮ A
   ...
   Under the assumption that the model is decidable we can
   prove lemma [[lemma:forcing-neg]]. {{{endproof}}}
