...
** Language
   <<sec:language>> The language of interpretability logics is $⟨⊥,→,▷⟩$. We
   define the usual operators and constants in the following way:
...
** Notation
   ...
   7. $⟦A⟧≔\{w:w⊩A\}$;
   8. {{{agda}}} this is Agda's logo. Each proof that is formalized in Agda
...
** Monotonicity
  ...
  frame with quasi-transitivity $(i)∈{1…8}$ (see [[sec:trans]]).
  ...
  Condition (2). Furthermore, for any valuation $V$ and formula $A$ with
  $M≔⟨F,V⟩$ and $M'≔⟨F',V⟩$ we have that for every world $w$,
  ...
  {{{beginremark}}} As Luka (or Mikec(?)) pointed out during online
  correspondence, taking the monotone closure of each $S_w$ is essentially
...
** Logic $IL$
  ...
  $Π⊢_{IL}A$ and $M$ is a generalized model with a world $w$ such that $w⊩Π$,
  then $w⊩A$. {{{endtheorem}}}

  {{{beginproof}}} {{{agda}}} We have verified this for all the presented
