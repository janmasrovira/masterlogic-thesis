...
#+latex_header: \newglossaryentry{dependent-pair}{name={dependent pair},description={A pair in which the type of the second component is indexed by the first}}
...
*** Generalized Veltman semantics
    ...
    5. if $uS_wY$ and $vS_wZ_y$ for all $y∈Y$, then $uS_w\left(⋃_{y∈Y}Z_y\right)$.
...
*** Generalized semantics
    ...
    \begin{flalign*}
    &∀w,x,u,V,K,𝔼(wRxRuS_wV, K∈𝒞(x,u) \\
    ⇒\ & (∃V'⊆V)(xS_wV',R[V']⊆K,(∀v∈V')(∀c∈K)(vRcSₓRₓ^{-1}[𝔼]⇒(∃𝔼'⊆𝔼)cS_v𝔼')))
    \end{flalign*}
    ...
    - \boxed{⇒} Let's fix the model and let $w ∈ W$ be arbitrary. Suppose $w⊩ A
      ...
      that there is a $V$, $uS_w V ⊩B$. Let $K ≔ \{c_Z: uS_x Z\}$. It is easy to
      check that $K ∈ 𝒞(x, u)$. Let $𝔼 ≔ [⊩E]$ (set of worlds that force $E$).
      For the selected $w, x, u, V, K, 𝔼$ the property $(R 1)_{gen}$ implies
      that there exists $V' ⊆ V$ such that:

      \[xS_wV',R[V']⊆K ,(∀v∈V')(∀c∈K)(vRcS_xR_x^{-1}[𝔼]⇒(∃𝔼'⊆𝔼)cS_v𝔼')\]
    ...
    - \boxed{⇐} Assume for a contradiction that $F⊭(R₁)_{gen}$. It follows that
      there exist $w,x,u,V,K,𝔼$ such that $wRxRuS_wV$, $K∈𝒞(x,u)$ and:
      \[(∀V'⊆V)(xS_wV', R[V']⊆K⇒ (∃v∈V')(∃c∈K)(vRcS_xR_x^{-1}[𝔼], (∀𝔼'⊆𝔼)
      c\cancel{S}_v 𝔼'))\]
      ...
      \[(∃v_U∈U)(∃c_U∈K)(v_URc_US_xR_x^{-1}[𝔼],(∀𝔼'⊆𝔼) c_U\cancel{S}_{v_U} 𝔼')\]
      ...
      the definition of $c_U$ we have $c_U S_x R_x^{−1} [𝔼]$, a forcing is defined
      such that $e$ is true exactly on the set $𝔼$. Hence $R_x^{-1}[𝔼]⊩♢e$ and thus
      $x ⊩ d▷♢e$.
      ...
      there exist $v_U,c_U$ such that $v_URc_US_xR_x^{-1}[𝔼],(∀𝔼'⊆𝔼)
      c_U\cancel{S}_{v_U} 𝔼'$. Since $c_U⊩d$ there must exist some $Z$ such that
      $c_US_{v_U}Z⊩e$, however, by the definition of the valuation it follows
      that $Z⊆𝔼$ and thus $c_U\cancel{S}_{v_U} Z$, which is a contradiction.
...
** Predicates and relations
   ...
   polymorphism out for simplicity.]:
   #+begin_src text
   Pred : Set → Set₁
   Pred A = A → Set
   #+end_src
...
data Pair (A B : Set) : Set where
  pair : A → B → Pair A B
    #+end_src
    Notice that =Pair= is a parameterized type as it has parameters =(A B :
...
data Σ (A : Set) (B : A → Set) : Set where
  Σ-pair : (a : A) → (b : B a) → Σ A (B a)
    ...
    #+end_src
    Let us prove that for every natural $n$ there exists some $m$ such that
    $n+1≤m$. Since we have already proven that $≤$ is reflexive the proof
    becomes trivial as we can pick $m≔n+1$:
    #+begin_src text
    n+1≤m : (n : Nat) → Σ Nat (λ m → n ≤ m)
    n+1≤m n = Σ-pair (suc n) (≤-refl (suc n))
    #+end_src
...
InfAscChain {W} _∼_ =
  ...
  → Σ (Pair W W) λ {(pair a b)
  → Pair (Pair (R a b) (f a b))
...
data _,_⊩_ (M : Model) (w : MW M) : Fm → Set where
  ...
  var : {x : Var} → V w x → M , w ⊩ var x
      #+end_src
   3. if $A,B∈Fm$, then $w⊩A▷B$ iff if $wRu$ and $u⊩A$ then there exists $v$ such
      that $v⊩B$ and $uS_wv$.
      #+begin_src text
   rhd : {A B : Fm} →
     ...
     → M , w ⊩ A ▷ B
      #+end_src
   4. if $A,B∈Fm$, then $w⊩A→B$ iff if $w⊩A$ then $w⊩B$;
      #+begin_src text
  impl : {A B : Fm} → ((M , w ⊩ A) → (M , w ⊩ B)) → M , w ⊩ (A ↝ B)
      #+end_src

   Unfortunately the definition above is not valid. The reason is that constructors
   =rhd= and =impl= both fail the positivity check.

   when we define a type in Agda, it cannot appear in a negative position in one
   of its constructors. since constructors =rhd= and =impl= are conflicting
