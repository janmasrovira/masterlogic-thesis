...
** Introduction to types
   ...
   type $A$. Later in this section we will see that types in simply typed lambda
   calculus provide a basic classification of lambda terms. For instance, a term
   representing a natural number will have a different type from a lambda term
   representing a Boolean value. In more expressive type theories such as
   dependent types theory we can express complex mathematical properties such as
   ``$2*n$ is always even'' or that ``any finite sequence of numbers can be
   sorted''.
...
**** Untyped lambda calculus
      ...
      - *Booleans*. We define /true/ and /false/ thus: \[true:=λa.λb.a;\ \ \
        ...
        conveniently allows us to use them as an /if then else/ expression. We
        can use this concept to encode the /and/ and /or/ operators. It may help
        to read the /and/ as ``if the first argument is true return the second
        argument else return false''. Likewise for the /or/. \[and:=λa.λb.a\ b\
        false;\ \ \ or≔λa.λb. a\ true\ b\]
...
**** Simply typed lambda calculus
      ...
      in a context we say that it is well-typed in that context.
      ...
      following the encoding given in the previous section considering a singleton
      set of base types $B≔\{α\}$.
      - *Booleans*. We define the type of Booleans thus: \[𝔹≔α→α→α\]
        ...
        $∅⊢and:𝔹→𝔹$ and that $∅⊢or:𝔹→𝔹$. As we can see, types give us
        information about the nature of the term. For instance, $and:𝔹→𝔹$ tells
        us that $and$ is a lambda term that expects a Boolean as an argument and
        returns a Boolean.
...
**** Dependently typed lambda calculus
     Briefly touch on $LF$.  (cite:loh2010tutorial).
