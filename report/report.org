...
* Frame conditions
  ...
  utility grows when it's applied to prove independence results between
  ...
  it's not completely obvious how to objectively justify why the original formulation is
  ...
  precisely define the minimal requirement (frame validity) for a frame condition,
  but comparing the usefulness of a frame condition remains open to
