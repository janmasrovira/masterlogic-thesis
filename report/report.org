...
# #+latex_header: \newunicodechar{𝕎}{\ensuremath{𝕎}}

#+latex_header: \newunicodechar{𝕎}{\makebox[.5em]{\myfont𝕎}}
# #+latex_header: \newunicodechar{𝕎}{\myfont{\ensuremath{𝕎}}}
...
** Subsets (predicates revisited)
   ...
   1. \boxed{∉} A proof of non membership is function from a proof of membership to =⊥=.
      #+begin_src text
      _∉_ : {A : Set} → A → Pred A → Set
      a ∉ X = ¬ (a ∈ X)
      #+end_src
   2. \boxed{⊆} A proof of inclusion =X ⊆ Y= is a function that maps a proof of
      membership to =X= to a proof of membership to =Y=.
      #+begin_example
      _⊆_ : {A : Set} → Pred A → Pred A → Set
      X ⊆ Y = ∀ {x} → x ∈ X → x ∈ Y
      #+end_example
   3. \boxed{∩} We use pairs to represent the intersection. Each component is a
      proof of membership to =X= and =Y= respectively.
      #+begin_src text
      _∩_ : {A : Set} → Pred A → Pred A → Pred A
      X ∩ Y = λ x → x ∈ X × x ∈ Y
      #+end_src
   4. \boxed{∪} We use a sum type to represent the union.
      #+begin_src text
      _∪_ : {A : Set} → Pred A → Pred A → Pred A
      X ∪ Y = λ x → x ∈ X ⊎ x ∈ Y
      #+end_src
