...
*** Contexts, and typing rules
   ...
   #+attr_latex: :float
    ...
    {{{begindef}}}
    <<def:agda-check-module>>
    *Type checking a module*. We understand by /type checking/ the process of
    checking that all terms and definitions in a module are correct with respect
    to the typing rules. We proceed by describing this process:
    1. At the beginning of a module we start with an empty context $Γ ≔ ∅$.
    2. We look at the next definition.
      - If it is a function definition of the
        form
        #+begin_example
        x : A
        x = t
        #+end_example
        We set $Γ'≔Γ;x:A$ and we check $Γ'⊢t:A$.

      - If it is a datatype definition of the form
          #+begin_example
      data D (x₁ : P₁) … (xₖ : Pₖ) : (y₁ : Q₁) → … → (yₗ : Qₗ) → Set ℓ where
        c₁ : T₁
        …
        ...
        Then we let \[Γ'≔Γ;D : (x₁ : P₁) → … → (xₖ : Pₖ) → (y₁ : Q₁) → … → (yₗ :
        Qₗ) → Set\ ℓ. \]

        We check that: ????
        1. $Γ'⊢D : (x₁ : P₁) → … → (xₖ : Pₖ) → (y₁ : Q₁) → … → (yₗ : Qₗ) → Set\ ℓ$
        2. and =cᵢ : Tᵢ= for every $i∈\{1,…,n\}$
        to the context.
