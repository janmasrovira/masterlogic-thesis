...
** A flawed proof in disguise
   ...
   \[M,w⊩A⇔M',f(w)⊩A\ \ \ \text{for every }w∈W,A∈\fm{}.\]
...
** Martin Löf's logical framework
   ...
   logic as /Martin Löf's logical framework/ or LF for short. {{{jan(needs a
   longer intro.)}}}
   ...
   \begin{mathpar}
   \inferrule[El]{A:\sf{Set} }{\sf{El}(A):\sf{Type}}
   \and
   \inferrule[El=]{A=B:\sf{Set} }{\sf{El}(A)=\sf{El}(B):\sf{Type}}
   \end{mathpar}
...
*** Syntax of types and terms (new)
    ...
    syntax of terms: \[S≔x\ |\ (x:S)↠S\ |\ c\] We want to highlight the
   ...
   S_{i-1}[x₁↦a₁,…,x_{i-2}↦a_{i-2}]:\sf{Type} &\text{ and } a_{i-1}:S_{i-1}[x₁↦a₁,…,x_{i-2}↦a_{i-2}]
    ...
    Then, if $Γ$ is a context, we write:
    1. $Γ⊢t:A$ to say that term $t$ has type
      $A$ in the context $Γ$. We write $t:A$ instead of $∅⊢t:A$. As expected, we
...
*** Examples
    Consider the fragment of intuitionistic logic with only implication. We
    identify formulas in this fragment with LF's types as follows:
    \begin{flalign*}
    p^* &≔ \sf{El}(P) & \text{where } P:\sf{Set} \\
    (A → B)^*&≔ A^* ↠ B^*
    \end{flalign*}
    # intuitionistic logic with the non-dependent
    # version of the $↠$ function type operator. We identify variables {{{jan(with
    # type variables??)}}}. That is we interpret $A→B$ as $A↠B$.
    ...
    # {λx→x:A↠A}
    # \end{mathpar}
    # We cannot fully complete the proof as we have no means of assuming that $A$
    # is a type within the formal derivation.
    Let us now show that $(p→p)^*$ is a theorem in LF. In order to prove
    that, we need to give a term with type $A↠A$. This term is in fact the
    identity function, as shown below.
    \begin{mathpar}
    \inferrule*[Left=Abs]
    {\inferrule*[Left=Assum]
    {\inferrule*[Left=El]
    ...
    {\sf{Set}:\sf{Type}}}
    {P:\sf{Set}⊢P:\sf{Set}}}
    {P:\sf{Set}⊢\sf{El}(P):\sf{Type}}}
    {P:\sf{Set},x:\sf{El}(P)⊢x:\sf{El}(P)}}
    {P:\sf{Set}⊢λx→x:\sf{El}(P)↠\sf{El}(P)}
    \end{mathpar}
    ...
    {\inferrule*[Left=Assum]
    {\inferrule*[Left=Set]{\ }
    {Q:\sf{Set},y:\sf{EL}(Q)⊢\sf{Set}:\sf{Type}}}
    {P:\sf{Set}, Q:\sf{Set},y:\sf{EL}(Q)⊢P:\sf{Set}}}
    {P:\sf{Set}, Q:\sf{Set},y:\sf{EL}(Q)⊢\sf{El}(P):\sf{Type}}}
    {P:\sf{Set}, Q:\sf{Set},x:\sf{EL}(P),y:\sf{EL}(Q)⊢x:\sf{El}(P)}}
    {P:\sf{Set}, Q:\sf{Set},x:\sf{EL}(P)⊢ λy→x:\sf{El}(Q)↠\sf{El}(P)}}
    {P:\sf{Set}, Q:\sf{Set}⊢λx→(λ y→ x):\sf{El}(P)↠(\sf{El}(Q)↠\sf{El}(P))}
    \end{mathpar}
    ...
    \inferrule*[Left=Abs]
    {…}
    {P:\sf{Set}, Q:\sf{Set}, R:\sf{Set}⊢λf→(λ g→ (λx→f\ x\ (g\ x))):\\\\
      (\sf{El}(P)↠(\sf{El}(Q)↠\sf{El}(R)))↠((\sf{El}(P)↠\sf{El}(Q))↠(\sf{El}(P)↠\sf{El}(R)))}
    \end{mathpar}

    Observe now that if $(p→q)^*$ and $p^*$ are theorems in LF, then $q^*$ is
    also a theorem. From the assumption it follows that there are terms
    $f:\sf{El}(P)↠\sf{El}(Q)$ and $a:\sf{EL}(P)$. Then we have that $f\ a$ is
    the desired term.
    \begin{mathpar}
    \inferrule*[Left=App]
    {\inferrule*[]{\ }{P:\sf{Set}, Q:\sf{Set}⊢f:\sf{El}(P)↠\sf{El}(Q)}
    \\ \inferrule*[]{\ }{P:\sf{Set}, Q:\sf{Set}⊢a:\sf{El}(P)}}
    {P:\sf{Set}, Q:\sf{Set}⊢f\ a: \sf{El}(Q) }
    \end{mathpar}
...
*** Types                                                         :noexport:
    So far we have explained the syntax of the theory, that is, how we can
    construct terms. In this section, we will explain the semantics of the
    theory. The semantics revolve around the concept of /type/, which plays the
    role of a proposition. First, we will define what is a type. ....

    What is a type?

    To denote that $S$ is a type we write $S:\sf{Type}$.

    Intuitively, a /context/, is a sequence of typed variables, where each type
    may depend on the preceding variables in the sequence.

    More precisely, a context is a finite sequence of the form
    \[x₁:S₁,…,xₙ:Sₙ\] Where $xᵢ$ denotes a variable[fn::Every variable must be
    different than the rest, that is, for every $i,j$ such that $i≠j$ we have
    $xᵢ≠xⱼ$.].
    Furthermore, for every $1≤i≤ n$ we have that given arbitrary terms
    $a₁,…,a_{i-1}$ such that
    \begin{flalign*}
   ...
   S_{i-1}[x₁↦a₁,…,x_{i-2}↦a_{i-2}]:\sf{Type} &\text{ and } a_{i-1}:S_{i-1}[x₁↦a₁,…,x_{i-2}↦a_{i-2}]
    ...
    Intuitively, a /type constructor/ (also known as /type family/),
    generalizes the concept of type in the sense that it is parameterized by a
    context. Note that a type constructor parameterized with an empty context is
    simply a type.

    More precisely, given a context $x₁:S₁,…,xₙ:Sₙ$ we say that $A$ is a type
    constructor and we write \[A[x₁:A₁,…,xₙ:Aₙ]:\sf{Type}\] if the
    following holds: Given arbitrary terms $a₁,…,aₙ$ that satisfy
    \[a₁:S₁,…,aₙ:Sₙ[x₁↦a₁,…,x_{n-1}↦a_{n-1}]\] then
    \[A[x₁↦a₁,…,xₙ↦aₙ]:\sf{Type}\].

    We require type constructors to be extensional with respect to the context.
    That is, if we have $A[x₁↦a₁,…,xₙ↦aₙ]:\sf{Type}$ and
    \begin{flalign*}
    a₁&=b₁:A₁ \\
    &… \\
    aₙ&=bₙ:Aₙ[x₁↦a₁,…,x_{n-1}↦a_{n-1}]
    \end{flalign*}
    then \[A[x₁↦a₁,…,xₙ↦aₙ]=A[x₁↦b₁,…,xₙ↦bₙ]\].

*** Defining new sets
    <<sec:def-new-types>>
**** Top
     We begin by defining the singleton type. We name it $⊤$. We say that it
     is a singleton type because there is only one term of this type, namely
     $\sf{tt}$. The $⊤$ type represents ``true'' in this theory.

     The constant that represents the type: \[⊤:\sf{Type}\] The
     only element in the type: \[\sf{tt}:⊤\] An induction principle:
     \[\sf{case}_{⊤}:(P : ⊤↠\sf{Type})↠P\ \sf{tt}↠(a:⊤)↠P\ a\] The
     above type should read as follows. Given a predicate $P$, a proof that $P$
     holds for the base case, that is, a term of type $P\ \sf{tt}$, and an
     arbitrary term $a$ of type $⊤$, we build a term of type $P\ a$, which is a
     proof that $a$ satisfies $P$.

     We also add the following equality.
     \begin{mathpar}
     \inferrule[]{P:⊤↠\sf{Type} \\ p : P\ \sf{tt}}
     {\sf{case}_{⊤}\ P\ p\ \sf{tt}=p\ \sf{tt}:P\ \sf{tt}}
     \end{mathpar}

**** Bottom
     We define the empty type, $⊥$, as we call it. We say that it is the empty
     type because there is no term of this type. The $⊥$ type represents
     ``false in this theory.

     We introduce the constant that represents the type: \[⊥:\sf{Type}\] We
     do not introduce any constants of this type. We introduce an induction
     principle:
     \[\sf{case}_⊥:(P : ⊥↠\sf{Type})↠(a:⊥)↠P\ a\]
     # ((Z : El ({}))Set, x : El ({})) El (Z(x))
     Notice how we can use the presented induction principle $\sf{case}_⊥$
     to build the type that corresponds to the principle /ex falso quodlibet/ or
     /principle of explosion/. \[\sf{exfalso}:(A:\sf{Type})↠(a:⊥)↠A\]
     The type of $\sf{exfalso}$ should read as follows. Given any
     proposition $A$ and a proof of $⊥$, we can build a proof of $A$.
     \jan{explain how}
**** Pairs
**** Disjoint unions
     Disjoint unions represent the disjunction according to the BHK
     interpretation. We introduce the type constructor: \[⊎ : Type↠Type↠Type\]
     We will use infix notation for $⊎$, hence we will write $A\ ⊎\ B$ instead
     of $⊎\ A\ B$.

     Since we want to represent two options, we introduce two constructors:
     \begin{flalign*}
     \sf{inj}₁ &: (A:Type)↠(B:Type)↠A↠ A\ ⊎\ B\\
     \sf{inj}₂ &: (A:Type)↠(B:Type)↠B↠ A\ ⊎\ B
     \end{flalign*}

     We introduce the following induction principle:
     \begin{flalign*}
     \sf{case}_⊎ &: (A:Type)↠(B:Type)↠(P : A\ ⊎\ B ↠ \sf{Type})\\
     &↠ ((a:A)↠P\ (\sf{inj}₁\ A\ B\ a)) \\
     &↠ ((b:B)↠P\ (\sf{inj}₂\ A\ B\ b)) \\
     &↠ (u:A\ ⊎\ B) \\
     &↠ P\ u
     ...
     \begin{mathpar}
     \inferrule[]{\sf{case}_⊎\ A\ B\ P\ p₁\ p₂\ (\sf{inj}₁\ A\ B\ a)=
     p₁\ a:P\ (\sf{inj}₁\ A\ B\ a)}{}
     \and
     \inferrule[]{\sf{case}_⊎\ A\ B\ P\ p₁\ p₂\ (\sf{inj}₂\ A\ B\ b)=
     p₂\ b:P\ (\sf{inj}₂\ A\ B\ b)}{}
     \end{mathpar}
...
**** Σ Pairs
     ...
     The introduce the type constructor for $Σ$ pairs: \[Σ : (A :
     \sf{Type})↠(B : A ↠ \sf{Type})↠\sf{Type}\] We introduce a
     constructor: \[\sf{pair}_Σ : (A : \sf{Type})↠(B : A ↠
     \sf{Type}) ↠ (a : A)↠(b : B\ a)↠ Σ\ A\ B\] The above constructor
     takes four arguments: The type of the first component, the type of the
     second component, the term for the first component, and the term for the
     second component.
     We add the following induction principle:
     \begin{flalign*}
     \sf{case}_Σ:&(A:\sf{Type}) ↠ (B : A↠\sf{Type})↠ (P : Σ\ A\ B↠\sf{Type}) \\
       &↠ ((a:A) ↠ (b:B\ a')↠P\ (\sf{pair}_Σ\ A\ B\ a\ b)) \\
       &↠ (u : Σ\ A\ B)\\
       &↠ P\ u
     \end{flalign*}
     We also add this equality.
     \begin{mathpar}
     \inferrule[]{A:\sf{Type}\\ B:A↠\sf{Type}\\P:Σ\ A\ B↠\sf{Type}\\\\
     p:(a':A)↠(b':B\ a')↠P\ (\sf{pair}_Σ\ A\ B\ a'\ b')\\\\
     a : A \\ b : B\ a
     }{
...
**** Natural numbers
**** Equality type
*** Equality of terms                                              :noexport:
     We now define when we consider two terms to be equal. We write $a≡b∼α$ to
     say that $a$ and $b$ are equal terms of arity $α$.
     1. /Equivalence/.
       \begin{mathpar}
       \inferrule[$≡$-Refl]{a∼α}{a≡a∼α}
       \and \inferrule[$≡$-Sym]{a≡b∼α}{b≡a∼α}
       \and \inferrule[$≡$-Trans]{a≡b∼α \\ b≡c∼α}{a≡c∼α}
       \end{mathpar}
     2. /Application/.
       \begin{mathpar}
       \inferrule[$≡$-App]{a≡a'∼α↠β \\ b≡b'∼α}{a\ b ≡ a'\ b'∼β}
       \and \inferrule[$≡$-$β$]{x∈\sf{Var}∼α \\  a∼α \\b∼β}{(λx→b)\ a≡b[x↦a]∼β}
       \end{mathpar}
       The \text{$≡$-App} tells us that we can replace equal terms in an
        application. The \text{$≡$-$β$} corresponds to the \text{$β$-reduction} rule
        in lambda calculus. In the \text{$≡$-$β$} rule we assume that no free
        variable in $a$ becomes bound in $b[x↦a]$.
     3. /Abstraction/.
       \begin{mathpar}
       \inferrule[$≡$-$ξ$]{x∈\sf{Var}∼α\\b≡b'∼β}{(λx→b)≡(λx→b')∼α↠β}
       \and \inferrule[$≡$-$α$]{x,y∈\sf{Var}∼ α \\ b∼β}{(λx→b)≡(λy→b[x↦y])∼α↠β}
       \\ \inferrule[$≡$-$η$]{x∈\sf{Var}∼ α \\ b∼α↠β}{(λx→b\ x)≡b∼α↠β}
       \end{mathpar}
       The \text{$≡$-$ξ$} rule tells us that we can replace equal bodies of
        lambda abstraction terms. The \text{$≡$-$α$} rule tells us that we can
        rename arguments of lambda abstraction terms. In this rule, we assume
        that $y$ does not occur free in $b$. The \text{$≡$-$η$} rule tells us
        that abstraction and application cancel each other. In this rule we
        assume that $x$ does not occur free in $b$.
     4. /Combination/.
       \begin{mathpar}
       \inferrule[$≡$-Comb1]{a₁≡a₁'∼α₁ \\ … \\ aₙ≡aₙ'∼αₙ}{a₁,…,aₙ≡a₁',…,aₙ'∼α₁⊗…⊗αₙ}
       \and \inferrule[$≡$-Comb2]{e∼α₁⊗…⊗αₙ}{π₁\ e,…,πₙ\ e≡e∼α₁⊗…⊗αₙ}
       \end{mathpar}
       The \text{$≡$-Comb1} rule tells us that we can replace equal terms in a
        combination. The \text{$≡$-Comb2} rule tells us that selection and
        combination cancel each other.
     5. /Selection/.
       \begin{mathpar}
       \inferrule[$≡$-Sel1]{a≡a'∼α₁⊗…⊗αₙ \\ 1≤i≤n}{πᵢ\ a≡πᵢ\ a'∼αᵢ}
       \and \inferrule[$≡$-Sel2]{a₁∼α₁ \\ … \\ aₙ∼αₙ \\ 1≤i≤n}{πᵢ\ (a₁,…,aₙ)≡aᵢ∼αᵢ}
       \end{mathpar}
        The \text{$≡$-Sel1} rule tells us that we can replace equal terms in a
        selection. The \text{$≡$-Sel2} rule tells us that selection projects the
        indicated component of a combination.
...
*** Arities                                                       :noexport:
    ...
    structure to the terms that we consider valid. For instance, arities reject
     terms such as $R(R)$. \jan{replace the quote with some explanation.}
     \begin{quote}
     ...
     1. $𝟬$ is an arity;
        \begin{mathpar}
        \inferrule{\ }{𝟬∈\sf{Arity}}
        \end{mathpar}
     2. if $α₁,…,αₙ$ (for $n≥2$) are arities, then $(α₁⊗…⊗αₙ)$ is an arity;
        \begin{mathpar}
        \inferrule{α₁∈\sf{Arity} \\ … \\αₙ ∈\sf{Arity}}{(α₁⊗…⊗αₙ)∈\sf{Arity}}
        \end{mathpar}
     3. if $α$ and $β$ are arities, then $(α↠β)$ is an arity.
        \begin{mathpar}
        \inferrule{α∈\sf{Arity} \\ β ∈\sf{Arity}}{(α↠β)∈\sf{Arity}}
        \end{mathpar}
     In BNF we have: \[A≔𝟬\ |\ (A⊗…⊗A)\ |\ (A↠A)\] We usually avoid
     parentheses by setting $⊗$ to have higher precedence than $↠$. Also, $↠$
     has right associativity. Thus, $𝟬↠ 𝟬↠ 𝟬⊕𝟬=(𝟬↠ (𝟬↠ (𝟬⊕𝟬)))$.

*** Syntax of terms :noexport:
     We proceed by defining terms in conjunction with their arities. We assume
     that every variable and every constant has a unique arity associated with
     it. We write $x∈\sf{Var}∼α$ to say that $x$ is variable of arity $α$.
     Likewise with constants.
     1. /Variable/. If $x$ is a variable of arity $α$, then $x$ is term of arity
       $α$;
       \begin{mathpar}
       \inferrule{x∈\sf{Var}∼α}{x∼α}
       \end{mathpar}
     2. /Constant/. If $c$ is a constant of arity $α$, then $c$ is a term of arity $α$;
       \begin{mathpar}
       \inferrule{c∈\sf{Const}∼α}{c∼α}
       \end{mathpar}
     # 3. /Defined identifier/. If $i$ is an identifier defined as $t$ and $t$ has
     #    arity $α$, then $i$ is a term of arity $α$;
     4. /Application/. If $f$ is term of arity $α↠β$ and $a$ is a term of
       arity $α$, then $f\ a$ is a term of arity $β$;
       \begin{mathpar}
       \inferrule{f∼α↠β \\ a∼α }{f\ a∼β}
       \end{mathpar}
     5. /Abstraction/. If $b$ is a term of arity $β$ and $x$ is a variable of
       arity $α$, then $(λx→b)$ is a term of arity $α↠β$. We may drop the
       parentheses when no ambiguity can occur;
       \begin{mathpar}
       \inferrule{x∈\sf{Var}∼α \\ b∼β}{(λx→b)∼α↠β}
       \end{mathpar}
     6. /Combination/. If $t₁,…,tₙ$ (for $n≥1$) are terms of arities $α₁,…,αₙ$
        respectively, then $⟨t₁,…,tₙ⟩$ is a term of arity $α₁⊗…⊗αₙ$;
       \begin{mathpar}
       \inferrule{t₁∼α₁\\ …\\ tₙ∼αₙ \\ n≥1}{⟨t₁,…,tₙ⟩∼α₁⊗…⊗αₙ}
       \end{mathpar}
     7. /Selection/. If $t$ is a term of arity $α₁⊗…⊗αₙ$ (for $n≥1$), then
       $πᵢ\ t$ is a term of arity $αᵢ$.
       \begin{mathpar}
       \inferrule{t∼α₁⊗…⊗αₙ \\ 1≤i≤n}{πᵢ\ t∼αᵢ}
       \end{mathpar}
