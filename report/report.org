...
#+latex_header: \newunicodechar{ₖ}{\makebox[0.7em]{\textsubscript{k}}}
#+latex_header: \newunicodechar{ₙ}{\makebox[0.7em]{\textsubscript{n}}}
#+latex_header: \newunicodechar{ₗ}{\makebox[0.7em]{\textsubscript{l}}}
...
** Introduction through the BHK interpretation
  ...
  (cite:sep-intuitionistic-logic-development) of intuitionistic logic. This
  first part will not use dependent types yet.
  ...
  the function using the =:= symbol as displayed below:
  #+begin_src text
  id : A → A
  id a = a
  #+end_src
  ...
  id a = a
  #+end_src
  With the technicalities out of the way, we can finally say that we have completed
  our first Agda proof according to the BHK interpretation.
...
data _×_ (A B : Set) : Set where
  _,_ : A → B → A × B
  #+end_src
  This definition defines a new type called =_×_= with parameters =(A B :
  Set)=. It has a single constructor named =_,_=. Underscores are used to denote
  infix operators.
  ...
  swap (a , b) = b , a
  #+end_src
  Note that we used /pattern matching/ to access the components of the pair.
  ...
  the code. This is not relevant here as they are just examples.]:
  #+begin_src text
  _ : {A B : Set} → A → B → A × B
  _ a b = a , b
  ...
  _ f (a , b) = f a b
  #+end_src
  Now that we are familiarized with product types we can proceed by exploring
  sum types.
...
*** data type definitions and constructors
    <<sec:agda-datatype>>
        #+begin_src text
