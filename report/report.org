...
** Martin Löf's logical framework
   ...
   propositions hold the status of theorem in LF. Such a notion is given by the
   following definition.
   #+begin_definition
   We say that some proposition (or type) $A$ is true in LF if there exists some
   $t$ such that: \[∅⊢t:A\]
   #+end_definition

   Before we give the typing rules of the system we need to define the concept
   of /context/.
   #+begin_definition
