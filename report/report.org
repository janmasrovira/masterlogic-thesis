...
** Monotonicity
  ...
  generalized Veltman frame that it satisfies the monotonicity condition and
...
*** BHK interpretation of propositional logic
   ...
   fitting way start the introduction is through the BHK interpretation of
   ...
   Let us identify propositional formulas restricted implication with Agda
   types. We define the map $*$ as follows:
   - If $p$ is a variable, then $p^*$ is =P=, where =P : Set=.
   - $(A→B)^*≔A^* \texttt{ → }B^*$. Note that the =→= on the left of the $≔$
     is Agda's arrow type.
   Then, if $F$ is a propositional formula with variables $p₁,…,pₙ$ we define
   \[
   ⟦F⟧≔ \texttt{\{P₁ : Set\} → … → \{P₁ : Set\} → F*}\] Note that in Agda we can
   write ={P₁ … Pₙ : Set} → F*= instead of ={P₁ : Set} → … → {P₁ : Set} → F*=.

   We will say that an Agda type =Prop= is a theorem if we can provide a definition
   ...
   commute f b a = f a b
   #+end_src
   We see that =commute= has three arguments, namely =f : A → B → C=, =b : B=
   and =a : A=. Then in the right hand side of the === sign we have =f a b=,
   which should read as ``term =f= applied to =a= and =b=''. Notice how function
   application is denoted by juxtaposition and has left associativity.

   Let us now put our attention on proofs that revolve around conjunction.
   First, we need to define introduce a new type. In Agda we can introduce new
 ...
 data _×_ (A B : Set) : Set where
   ...
   swap (a , b) = b , a
   #+end_src
   Note that we used pattern matching to deconstruct the term and access the
   components of the pair. Observe that when we deconstruct an argument using
   pattern matching we need put parenthesis around the argument.

   We proceed by giving some examples involving conjunction.

   We can show that $⟦A→(B→A∧B)⟧$ is an theorem in Agda by simply applying the
   constructor =_,_=:
   #+begin_src text
   p1 : {A B : Set} → A → B → A × B
   p1 a b = a , b
   ...
   We show that we can prove $⟦A∧B→A⟧$ and $⟦A∧B→B⟧$. We need to pattern match
   on the argument to access to corresponding component and return it.
   #+begin_src text
     proj₁ : {A B : Set} → A × B → A
     proj₁ (a , b) = a
 ...
 data _⊎_ (A B : Set) : Set where
   ...
   =A=. The constructor =inj₂= builds a term of type =A ⊎ B= given a proof of =B=.
   ...
   p5 (inj₂ b) f g = g b
   #+end_src
   The reader may have raised the following question in their mind:
