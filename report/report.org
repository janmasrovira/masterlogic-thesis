...
** Martin Löf's logical framework
   ...
   cite:nordstrom1990programming. \jan{needs a longer intro.}
   ...
   \begin{mathpar}
   \inferrule[]{\ }{\mathsf{Set}:\mathsf{Type}}
   \end{mathpar}

   If $A$ is a set, then the elements of $A$ in conjunction with their
   equivalence relation forms a type.
   \begin{mathpar}
   \inferrule[]{A:\sf{Set} }{\sf{El}(A):\sf{Type}}
   \and
   \inferrule[]{A=B:\sf{Set} }{\sf{El}(A)=\sf{El}(B):\sf{Type}}
   \end{mathpar}
...
*** Syntax of types and terms (new)
    ...
    syntax of terms: \[S≔x\ |\ S\ S\ |\ (x:S)↠S\ |\ c\] We want to highlight the
   ...
   S_{i-1}[x₁↦a₁,…,x_{i-2}↦a_{i-2}]:\sf{Type} &\text{ and } a_{i-1}:S_{i-1}[x₁↦a₁,…,x_{i-2}↦a_{i-2}]
    ...
    \begin{mathpar}
    \inferrule[Assum]{Γ⊢A:\sf{Type}\\x:A∈Γ}
    {Γ⊢x:A}
    \end{mathpar}
    ...
    \begin{mathpar}
    \inferrule[Refl]{A:\sf{Type}}{A=A} \and
    \inferrule[Sym]{ A=B:\sf{Type} }{B=A:\sf{Type}} \and
    \inferrule[Trans]{A,B:\sf{Type} \\ A=B:\sf{Type} \\ B=C:\sf{Type}}{A=C:\sf{Type}}
    \end{mathpar}
    Furthermore we can substitute equal types.
    \begin{mathpar}
    \inferrule[Id1]{A,B:\sf{Type} \\a:A \\ A=B}{a:B} \and
    \inferrule[Id2]{A,B:\sf{Type} \\a=b:A\\A=B}{a=b:B}
    \end{mathpar}
...
*** Function types
    The type of functions from a type $A$ to a type $B$ indexed by a term of
    type $A$.
    \begin{mathpar}
    \inferrule[]{Γ⊢A:\sf{Type}\\Γ;x:A⊢B:\sf{Type}}{Γ⊢(x:A)↠B:\sf{Type}}
    \end{mathpar}
    As a notational convention, when $B$ does not depend on a term of type $A$,
    we will simply write $A↠B$ instead of $(x:A)↠B$.

    Substitution in function types:
    \begin{mathpar}
    \inferrule[]{Γ⊢A':\sf{Type}\\ Γ⊢A':\sf{Type}\\Γ⊢A=A'\\\\
     B[x:A]:\sf{Type}\\B'[x:A]:\sf{Type}\\B[x:A]=B'[x:A]}
     {(x:A)↠B=(x:A')↠B'}
    \end{mathpar}
    Function application and substitution.
    \begin{mathpar}
    \inferrule[App]{c:(x:A)↠B \\ a:A}{c\ a:B[x↦a]}
    \and \inferrule[]{c=c':(x:A)↠B \\ a=a':A}{c\ a=c'\ a'=:B[x↦a]}
    \end{mathpar}
    Abstraction.
    \begin{mathpar}
    \inferrule[Abs]{b:B[x:A]}{λx→b:(x:A)↠B}
    \end{mathpar}
    The rule of \text{$β$-reduction}.
    \begin{mathpar}
    \inferrule[$β$-$=$]{a:A \\ b:B[x:A]}{(λx→b)\ a=b[x↦a]:B[x↦a]}
    \end{mathpar}
    The \text{$ξ$-$=$} rule tells us that we can replace equal bodies of lambda
    abstraction terms.
    \begin{mathpar}
    \inferrule[$ξ$-$=$]{b=b':B[x:A]}{λx→b=λx→b':(x:A)↠B}
    \end{mathpar}
...
*** Type constructors and constructors
    We say that $C$ is a type constructor if it is a constant with a type of the
    form \[C:(a₁ : A₁) ↠…↠ (aₙ:Aₙ) ↠ \sf{Type}\ \ \ \text{ for } n≥0\] As
    examples of type constructors consider constant $ℕ:\sf{Type}$, which
    denotes the type of natural numbers or the type of dependent products
    $Σ:(A : \sf{Type})↠(B : A↠\sf{Type})↠\sf{Type}$. We define the
    mentioned types, among others, in full detail in Section [[sec:def-new-types]].
