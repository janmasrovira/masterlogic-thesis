...
** BHK interpretation of first order logic
   ...
   1. A proof of $∀x.F(x)$ is a function that given an arbitrary element $c$ in
      the domain, builds a proof that $c$ satisfies $F$.
   2. A proof of $∃x.F(x)$ is a witness $c$ in the domain and a proof that $c$
     satisfies $F$.
...
-- Alternatively we could have written this:
s≠z : ∀[ Nat ] (λ n → ¬ (suc n ≡ zero))
s≠z = proof∀ λ {n ()}
   #+end_src
   Implementing $∀$ elimination (if $∀x.Px$ and $c$ is in the domain then
