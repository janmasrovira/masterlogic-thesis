...
** Monotonicity
  ...
  - Assume that $M,w⊮A▷B$, it follows that there is some $x$ such that $wRx⊩A$
    and $(⋆)\ ∀Y(xS_wY⇒Y⊮B)$. We want to prove that $∀Y'(xS'_wY'⇒Y'⊮B)$. Assume
    that for some $Y'$ we have $xS'_wY'$, by definition of $S'$ it follows there
    exists some $Y$ such that $Y⊆Y'$ and $xS_wY$. Hence by $(⋆)$ we have that
    $Y⊮B$ and thus there exists $y∈Y$ such that $y⊮B$, therefore $Y'⊮B$ and $M',w⊩A▷B$.
