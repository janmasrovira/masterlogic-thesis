...
WHERE DO YOU EXPLAIN HOW TO READ YOUR DIAGRAMS?
TO NOT OVERLOAD WRITING, SHALL WE SPEAK OF FRAMES WHEN SPEAKING OF REGULAR/ORDINARY FRAMES AND ONLY INDICATE GENERALISED WHERE NEEDED?
LUKA, WHAT DO YOU THINK?

LM: I agree, that's what Croatian authors do
...
HERE AND IN THE THESIS IN GENERAL, I MISS A DISCUSSION ABOUT ASSURINGNESS. THIS
...
LM: Joost, would you use assuringness/criticality even in semantic context? I
...
YOUR CURRENT SECTION 1 SHOULD BE SPLIT INTO AN INTRODUCTION AND A SECTION(S) WHERE YOU DISCUSS THE NEEDED/DESIRED PRELIMINARIES. WHAT TO PUT INTO THE PRELIMINARIES. THE LOWER-BOUND IS WHAT IS NEEDED TO READ THE THESIS. TYPICALLY YOU INCLUDE MORE BASED ON YOUR PERSONAL CRITERION. PERSONALLY I WOULD INCLUDE THINGS ON THE BASIS OF TWO CRITERIA: 
(1) THE THINGS THAT YOU GET EXCITED ABOUT AND YOU WISH TO SHARE THAT BEAUTY WITH THE WORLD/READING COMMITTEE.
(2) THINGS YOU WANT TO SHOW OFF WITH AND SHOW THE COMMITTEE THAT YOU LEARNED AND UNDERSTOOD COOL AND DIFFICULT THINGS.
...
* Verbrugge
  Let $M≔⟨W, R, S, V⟩$ be a generalized Veltman that satisfies
  quasi-transitivity Condition 4.
  We define an ordinary Veltman frame $M'≔⟨W',R',S',V'⟩$.
  \begin{flalign*}
  W'≔&\{⟨x,A⟩:A⊆W^2, \\ &(W1)\ ∀⟨u,v⟩∈A\ ∃Y(xS_uY,v∈Y), \\
  & \textcolor{red}{(W2')\ ∀uV(xS_uV⇒∃v∈V(⟨u,v⟩∈A),} \\
  & (W2)\ ∀uV((xS_uV,v∈V)⇒⟨u,v⟩∈A\} \\
  R'≔&\{⟨⟨x,A⟩,⟨y,B⟩⟩ : xRy,∀wz(wRx⇒⟨w,z⟩∈B⇒⟨w,z⟩∈A)\} \\
  S'≔&\{⟨⟨w,C⟩,⟨x,A⟩,⟨y,B⟩⟩ : ⟨w,C⟩R'⟨x,A⟩,⟨w,C⟩R'⟨y,B⟩, (S1) ∀v(⟨w,v⟩∈B⇒⟨w,v⟩∈A) \} \\
  ...
  \[w⊩D⇔⟨w,C⟩⊩D\]
  {{{endtheorem}}}
  {{{beginproof}}} We proceed by induction on the formula. We only consider the
  case $D▷E$ as the other cases are easy.
  - \boxed{⇒} {{{agda}}} Assume $w⊩D▷E$ and pick some $A$ such that $⟨x,A⟩∈W'$. We want to
    prove $⟨x,A⟩⊩D▷E$. Assume that for some $⟨x,B⟩∈W'$ we have $⟨w,C⟩R'⟨x,A⟩⊩D$.
    By IH it follows that $x⊩D$ and hence there exists $V$ such that $xS_wV⊩E$.
    By quasi-transitivity Condition (4) on $xS_wV$ we have that there exists
    $y∈V$ such that
    \[  (E1)\ \ ∀Z(yS_wZ⇒xS_wZ)   \]
    Pick any $y∈V$ and any $B$ such that $⟨y,B⟩∈W'$. By IH we have $⟨y,B⟩⊩E$ so
    it remains to show that $⟨x,A⟩S'_{⟨w,C⟩}⟨y,B⟩$:
    1. $⟨w,C⟩R'⟨x,A⟩$ is true by assumption.
    2. For $⟨w,C⟩R'⟨y,B⟩$ we show that:
       - $wRy$. Since $y∈V$ and $xS_wV$ we have $wRy$.
       - $∀bz(bRw⇒⟨b,z⟩∈B⇒⟨b,z⟩∈C)$. Assume that for some $b,z$ we have
         $⟨b,z⟩∈B$. By $(W2)$ for $C$ it suffices to show that there exists some
         $Z$ such that $wS_bZ$ with $z∈Z$. By $(W1)$ for $B$ it follows that
         there exists $Z$ such that $yS_bZ$ with $z∈Z$. Since $bRwRy$ we have
         $wS_b\{y\}$ and by quasi-transitivity (4) we have that
         $∀Y(yS_bY⇒wS_bY)$. Since $yS_bZ$ it follows that $wS_bZ$ and
         hence $Z$ is the desired set.
    3. $∀v(⟨w,v⟩∈B⇒⟨w,v⟩∈A)$. Assume that for some $v$ we have $⟨w,v⟩∈B$. By
       $(W2)$ for $A$ it suffices to show that there is some $Y$ such that
       $xS_wZ$ and $v∈Z$. By $(W1)$ for $B$ it follows that there exists $Z$ with
       $yS_wZ$ and $v∈Z$. Finally by $(E1)$ it follows that $xS_wZ$.
  - \boxed{⇐}
  {{{endproof}}}
* other
