...
*** Ordinary Veltman semantics
    ...
    (cite:modal-matters) An ordinary Veltman frame $F=⟨W,R,S⟩$ is a
    ...
    {{{begindef}}} An ordinary Veltman model $M=⟨F,V⟩$ is a structure
    ...
    {{{begindef}}}
    <<def:ord-forcing>>
    Given a model $M$, we define a forcing relation $⊩^{ord}⊆W × Fm$. We write
    $M,w⊩A$ instead of $⟨w,A⟩∈⊩^{ord}$ or simply $w⊩A$ when the model is clear by
...
*** Generalized Veltman semantics
    ...
    {{{begindef}}}
    <<def:gen-frame>>
    A generalized Veltman model $M=⟨F,V⟩$ is a structure
    constituted by a generalized Veltman frame $F$ and a valuation $V⊆W×Var$.
    {{{enddef}}}
    {{{begindef}}}
    Given a model $M$, we define a forcing relation $⊩^{gen}_M⊆W ×
    Fm$. We use the same notation conventions as in the ordinary semantics.
    The definition is the same as Definition [[def:ord-forcing]] except for the $▷$ case:
