...
data _⊢_ (Π : List Fm) : Fm → Set where
  ...
  nec : ∀ {A} → [] ⊢ A → Π ⊢ □ A
   ...
   Consider the syntactic proof of $A→A$.
   #+begin_src text
⊢A↝A : ∀ {A Π} → Π ⊢ A ↝ A
...
⊢A▷A {A} = MP J1 (nec ⊢A↝A)
   ...
   human to understand them with the syntax used above. This motivates our next
   section, in which we present a way to express syntactic proofs in Agda using
   paper-like syntax.
...
*** An eDSL
     ...
     cite:slug for logic $K$ and what follows is an adaptation of that.
