...
** The principle \prin{M}
   ...
   The theorems of $\sf{ILM}$ are the set of interpretability principles that
   are always provable in theories which are $Σ_1$ sound, have full induction and
   prove consistency of any of its finite subsystems (cite paper)
   (cite:visser1997overview). An example of such theory is $\sf{PA}$.
*** Ordinary semantics
...
* The logic of Agda
  ...
  In this section we give an informal overview of the basic Agda constructions
  that will hopefully help the reader get an intuition of how dependent types
  can be used to prove mathematical properties. We will also highlight some
  intricacies of the language that are relevant to our project. This is not
  meant to be an exhaustive analysis of the inner workings of Agda, as this
  falls out of the scope of this project. The original author of Agda, Ulf
  Norell, has suggested[fn::In correspondence via email.]
  cite:cockx2018elaborating as a good reference for that matter.

** BHK interpretation of propositional logic
  Agda is based on an intuitionistic type theory with dependent types that
  extends Per Martin-Löf's type theory (cite:martin1984intuitionistic). Agda's
  ...
  According to the BHK interpretation a proof of $A→A$ is just the identity
  ...
  arguments to the left of the === sign. Furthermore, we should write the type
  of the function using the =:= symbol. The =:= symbol denotes the typing
  relation, hence =a : A= reads as ``term =a= has type =A= ''. We use the symbol
  =→= for functions.
  #+begin_src text
  id : A → A
  id a = a
  ...
  completed our first Agda proof of $A→A$ according to the BHK interpretation.
  ...
  should read as ``term =f= applied to =a= and =b=''. Notice how function
  application is denoted by simple juxtaposition.
  ...
  terms the type of a tuple. Consider the following pair definition:
  #+begin_src text
data _×_ (A B : Set) : Set where
  _,_ : A → B → A × B
  #+end_src
  This definition defines a new datatype called =_×_= with parameters =(A B : Set)=.
  ...
  constructor =_,_= is =A → B → A × B=. This tells us that =_,_= is a constructor
...
data _⊎_ (A B : Set) : Set where
  ...
  explosion ()
  #+end_src
  Since the =⊥= type has no constructors, we get the empty pattern when pattern
  matching against the argument of type =⊥=.
...
** Booleans and case analysis
   ...
   In this section we show how we can define a datatype that represents that
   concept and we give a small introduction to case analysis through pattern
   matching.
   ...
   see Section [[sec:agda-equality]].]. We use the symbol
...
** Universe hierarchy
   ...
   i+1= is true while =Set i : Set i+n= does not hold for $n>1$. In Agda we write
   ...
   level and =lsuc= for the successor level), but it only works as a universe
...
data _×_ (A B : Set) : Set where
  _,_ : A → B → A × B
   ...
   the $Σ$ type (or dependent pair\glspl*{dependent-pair}) comes into play. A
...
syntax Σ-syntax A (λ x → B) = Σ[ x ∈ A ] B
   ...
   would have =∃[ c ] (P c)=. We use this notation, for instance, in the
   definition of Veltman semantics in Section [[sec:agda-ord-semantics]].
   See again the =∃-intro= theorem type using this notation:
   #+begin_src text
   ∃-intro : {ℓ ℓ' : Level} {A : Set ℓ} {P : A → Set ℓ'}
     → (a : A) → P a → ∃[ c ] (P c)
   #+end_src
...
** Equality
   ...
   equality for Booleans and naturals. In this section we explore
   polymorphic equality and some of its properties and limitations.

   Below we present a slight simplification of equality as defined in
   cite:agda-stdlib.
   #+begin_example
data _≡_ {a : Level} {A : Set a} (x : A) : A → Set a where
  refl : x ≡ x
   ...
   We give a description of the polymorphic equality datatype defined above taken
   from cite:plfa2019: For any type =A= and for any =x= of type =A=, the
   constructor =refl= provides evidence that =x ≡ x=. Hence, every value is
   equal to itself, and we have no other way of showing values equal. The
   definition features an asymmetry, in that the first argument to =_≡_= is
   given by the parameter =x : A=, while the second is given by an index in =A →
   Set a=. The first argument to =_≡_= can be a parameter because it does not
   vary, while the second must be an index, so it can be required to be equal to
   the first.

   It is easy to see that it is a reflexive relation.
   #+begin_src text
...
reflexive = refl
   ...
   #+begin_src text
symmetric : ∀ {ℓ} {A : Set ℓ} {a b : A} → a ≡ b → b ≡ a
symmetric a≡b = ?
   #+end_src
...
*** Extensionality
    <<sec:agda-ext>>
    A necessary question is ``can we define equality of sets?''.
** Positivity
   <<sec:positivity>>
...
□_ a = ¬' a ▷ ⊥'
  ...
  code defines the /infixity/ of =_↝_= and =_▷_=
    #+begin_src text
  infixr 20 _↝_
  infixr 50 _▷_
    #+end_src
...
** Generalized semantics
   ...
   semantics in Agda. As explained in Section [[sec:trans]] we consider eight
   different quasi-transitivity properties, thus, we need to define everything
   related to generalized Veltman semantics to be generic with respect to the
   quasi-transitivity condition used, which was certainly a challenge.
...
*** A guided Agda proof
    ...
    Löb's axiom is in $\sf{IL}$ we need to show this as part of the soundness proof.
