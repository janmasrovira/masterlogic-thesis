...
** Basic Agda
   In this section we precisely define a moderate subset of Agda.
   ...
   #+attr_latex: :align llll :float t :center t :placement [H]
    ...
    |        | \vert | =(iden : term) → term=               | function type                             |
    |        | \vert | =λ iden → term=                      | lambda abstraction;                       |
    |        | \vert | =λ {pat₁ → term₁; … ; patₙ → termₙ}= | lambda abstraction with pattern matching; |
...
*** Contexts, and typing rules
    ...
    *Notation*. We use $Γ;t:A$ as short for $Γ∪\{⟨t:A⟩\}$.
    ...
    {{{begindef}}} *Well-typed term*. We say that a term $t$ is well-typed in
    context $Γ$ if $Γ⊢t:A$ for some type $A$. The rules for $⊢$ are presented
    below.
   ...
   #+attr_latex: :float
    ...
    \and
    \inferrule[Function]{Γ ⊢ A:Set\ i \\ x:A;Γ ⊢ B : Set\ i}{Γ ⊢ (x:A)→B:Set\ i}
    \\
    ...
    module are well-typed and respect the typing annotations. By respecting the
    typing annotations we mean that if we have the following definition:
    #+begin_example
    x : A
    x = t
    #+end_example
    Then we need check that =t= is well-typed and furthermore has type =A=. It
    may be worth noticing that sometimes the annotation =x : A= may be redundant
    since the type of =t= can be automatically inferred to be =A= from the
    rules. However, for simplicity we assume that the annotation is always
    required.
    ...
    2. We look at the next definition.
       - If it is a *function definition*, it is of the
         form
         #+begin_example
         x : A
         x = t
         #+end_example
         We let $Γ'≔Γ;x:A$ and we check $Γ'⊢t:A$.
       ...
       data D (x₁ : P₁) … (xₖ : Pₖ) : (y₁ : Q₁) → … → (yₗ : Qₗ) → Set ℓ where
         ...
         We check that:
         1. For each parameter =xᵢ : Pᵢ= we check that for some $ε≤ℓ$ we have
    ...
    3. We repeat the process for the next definition until we reach the end of
       the module.
    {{{enddef}}}
*** Normalization rules
    wip
