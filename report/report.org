...
data _≡_ : Bool → Bool → Set where
  ...
  f≡f : false ≡ false
   ...
   #+begin_src text
   _ : (b : Bool) → and false b ≡ false
   _ b = f≡f

   _ : (b : Bool) → and b false ≡ false
   _ true = f≡f
   _ false = f≡f
   #+end_src
...
** Naturals and recursion
   ...
   data _≡_ : Nat → Nat → Set where
     z≡z : zero ≡ zero
     s≡s : {n : Nat} → n ≡ n → suc n ≡ suc n
   #+end_src
   We see that it has a similar structure to the datatype for Boolean equality.
   The only difference is that the =s≡s= constructor requires a proof of =n ≡ n=
   ...
   #+end_src
   While the types match we see that we perform a recursive call on the same
   argument and thus we get an infinite loop. Agda has a termination checker
   that rejects proofs where termination cannot be assured and thus rejects the
   previous definition.
